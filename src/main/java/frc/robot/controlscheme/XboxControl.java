...
public class XboxControl implements ControlScheme {
    ...
    */
    ...
    public ShuffleboardCommand getShuffleboardCommand(PowerDistributionPanel pdp, LiftSubsystem liftSubsystem, PneumaticsSubsystem pneumaticsSubsystem, CANSparkMax[] canSparkMaxs, TalonSRX[] talonSRXs, CANSparkMax cascadeSpark, PID cargoPID, PID drivePID, double[] maxOmegaGoal) {
        return new ShuffleboardCommand(pdp, liftSubsystem, pneumaticsSubsystem, canSparkMaxs, talonSRXs, cascadeSpark, getButton(XboxControlButton.Button.BUMPER_LEFT), getButton(XboxControlButton.Button.BUMPER_RIGHT), getButton(XboxControlButton.Button.A), cargoPID, drivePID, maxOmegaGoal);
    }
