...
public class LidarSubsystem extends Subsystem {
    ...
    public Line pointSlopeForm(Point p, double m){return new Line(m, p1.y - m * p1.x);}
    ...
    public Point intersection(Line l1, Line l2)  {return pointOfX((l1.b - l2.b)/(l2.m - l1.m));}
    public Line perpindicular(Line l, Point p)   {return pointSlope(p, -1/l.m);}
    ...
    public Hashtable<String,Integer> isHatch(Point[] points){
        ...
        data.add("detected", (isLeftWall && isRightWall & (offWallEnd >= rightWallEnd)) ? 1 : 0);
        if (data.get("detected") == 0) {return data;}
        Point hatchCenter = center(points[rightWallEnd],points[leftWallEnd]);
        data.add("shift", hatchCenter.x);
        data.add("parallel", hatchCenter.y);
        data.add("angle", Math.atan(wall.m));
        return data;
    }

    public Hashtable<String,Integer> hatchInfo(double[] distances, double leftTheta, double rightTheta){
        double angleOn = firstAngle;
        Point[] points = new Point[(int) (rightTheta - leftTheta)/deltaTheta];
        while(angleOn < leftTheta){angleOn++;}
        while(angleOn < rightTheta){
            points[angleOn] = toPoint(distance[angleOn],angleOn);
            angleOn++;
        }
